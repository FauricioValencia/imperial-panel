...
```sql
 SELECT ci.courier_id,
    ...
    bool_or(pl.expires_at IS NOT NULL AND pl.expires_at < (now() + '7 days'::interval) AND ci.quantity_remaining > 0) AS has_expiring_soon
...
```sql
 SELECT id AS product_id,
    ...
    codigo,
    price,
    stock AS warehouse_total_physical,
    stock_available AS warehouse_available,
    COALESCE(( SELECT sum(ci.quantity_remaining) AS sum
           FROM courier_inventory ci
             JOIN product_lots pl ON pl.id = ci.lot_id
          WHERE ci.product_id = p.id AND ci.quantity_remaining > 0 AND pl.active = true AND (pl.expires_at IS NULL OR pl.expires_at > now())), 0::bigint) AS in_couriers_available,
    COALESCE(( SELECT sum(ci.quantity_remaining) AS sum
           FROM courier_inventory ci
          WHERE ci.product_id = p.id AND ci.quantity_remaining > 0), 0::bigint) AS in_couriers_total,
    stock_available + COALESCE(( SELECT sum(ci.quantity_remaining) AS sum
           FROM courier_inventory ci
             JOIN product_lots pl ON pl.id = ci.lot_id
          WHERE ci.product_id = p.id AND ci.quantity_remaining > 0 AND pl.active = true AND (pl.expires_at IS NULL OR pl.expires_at > now())), 0::bigint) AS available_global
   FROM products p
  WHERE active = true;
```
...
```sql
 SELECT pl.product_id,
    ...
    COALESCE(( SELECT sum(courier_inventory.quantity_remaining) AS sum
           FROM courier_inventory
          WHERE courier_inventory.lot_id = pl.id), 0::bigint) AS in_couriers,
    COALESCE(( SELECT sum(outbound_lot_allocations.quantity) AS sum
           FROM outbound_lot_allocations
          WHERE outbound_lot_allocations.lot_id = pl.id), 0::bigint) AS allocated_to_orders,
    pl.quantity_received - pl.quantity_remaining - COALESCE(( SELECT sum(courier_inventory.quantity_remaining) AS sum
           FROM courier_inventory
          WHERE courier_inventory.lot_id = pl.id), 0::bigint) - COALESCE(( SELECT sum(outbound_lot_allocations.quantity) AS sum
           FROM outbound_lot_allocations
          WHERE outbound_lot_allocations.lot_id = pl.id), 0::bigint) AS drift
   FROM product_lots pl
     JOIN products p ON p.id = pl.product_id
...
| courier_inventory | 2200_41909_1_not_null | CHECK |  |
| courier_inventory | 2200_41909_2_not_null | CHECK |  |
| courier_inventory | 2200_41909_3_not_null | CHECK |  |
| courier_inventory | 2200_41909_4_not_null | CHECK |  |
| courier_inventory | 2200_41909_5_not_null | CHECK |  |
| courier_inventory | 2200_41909_6_not_null | CHECK |  |
| courier_inventory | 2200_41909_7_not_null | CHECK |  |
| courier_inventory | 2200_41909_8_not_null | CHECK |  |
| courier_inventory | courier_inventory_quantity_remaining_check | CHECK | CHECK ((quantity_remaining >= 0)) |
| courier_inventory | courier_inventory_admin_id_fkey | FOREIGN KEY | admin_id -> users(id) |
| courier_inventory | courier_inventory_courier_id_fkey | FOREIGN KEY | courier_id -> users(id) |
...
| delivery_attempts | 2200_42045_1_not_null | CHECK |  |
| delivery_attempts | 2200_42045_2_not_null | CHECK |  |
| delivery_attempts | 2200_42045_3_not_null | CHECK |  |
| delivery_attempts | 2200_42045_4_not_null | CHECK |  |
| delivery_attempts | 2200_42045_5_not_null | CHECK |  |
| delivery_attempts | 2200_42045_6_not_null | CHECK |  |
| delivery_attempts | delivery_attempts_admin_id_fkey | FOREIGN KEY | admin_id -> users(id) |
...
| inventory_movements | chk_muestra_requiere_cliente | CHECK | CHECK (((reason <> 'muestra'::text) OR (sample_customer_id IS NOT NULL))) |
| inventory_movements | inventory_movements_type_check | CHECK | CHECK ((type = ANY (ARRAY['inbound'::text, 'outbound'::text, 'return'::text, 'adjustment'::text, 'transfer_out'::text, 'transfer_in'::text]))) |
| inventory_movements | inventory_movements_admin_id_fkey | FOREIGN KEY |  |
| inventory_movements | inventory_movements_courier_id_fkey | FOREIGN KEY | courier_id -> users(id) |
...
| inventory_movements | inventory_movements_transfer_id_fkey | FOREIGN KEY | transfer_id -> stock_transfers(id) |
...
| pending_inventory_adjustments | 2200_42006_1_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_2_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_3_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_5_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_6_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_7_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_8_not_null | CHECK |  |
| pending_inventory_adjustments | 2200_42006_9_not_null | CHECK |  |
| pending_inventory_adjustments | pending_inventory_adjustments_quantity_check | CHECK | CHECK ((quantity > 0)) |
| pending_inventory_adjustments | pending_inventory_adjustments_status_check | CHECK | CHECK ((status = ANY (ARRAY['pending'::text, 'resolved'::text, 'cancelled'::text]))) |
| pending_inventory_adjustments | pending_inventory_adjustments_admin_id_fkey | FOREIGN KEY | admin_id -> users(id) |
| pending_inventory_adjustments | pending_inventory_adjustments_courier_id_fkey | FOREIGN KEY | courier_id -> users(id) |
| pending_inventory_adjustments | pending_inventory_adjustments_order_item_id_fkey | FOREIGN KEY | order_item_id -> order_items(id) |
| pending_inventory_adjustments | pending_inventory_adjustments_product_id_fkey | FOREIGN KEY | product_id -> products(id) |
| pending_inventory_adjustments | pending_inventory_adjustments_resolved_by_fkey | FOREIGN KEY | resolved_by -> users(id) |
| pending_inventory_adjustments | pending_inventory_adjustments_pkey | PRIMARY KEY | id |
...
| stock_transfer_lines | 2200_41975_1_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_2_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_3_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_4_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_5_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_6_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_7_not_null | CHECK |  |
| stock_transfer_lines | 2200_41975_8_not_null | CHECK |  |
| stock_transfer_lines | stock_transfer_lines_quantity_check | CHECK | CHECK ((quantity > 0)) |
| stock_transfer_lines | stock_transfer_lines_admin_id_fkey | FOREIGN KEY | admin_id -> users(id) |
| stock_transfer_lines | stock_transfer_lines_lot_id_fkey | FOREIGN KEY | lot_id -> product_lots(id) |
| stock_transfer_lines | stock_transfer_lines_product_id_fkey | FOREIGN KEY | product_id -> products(id) |
| stock_transfer_lines | stock_transfer_lines_transfer_id_fkey | FOREIGN KEY | transfer_id -> stock_transfers(id) |
| stock_transfer_lines | stock_transfer_lines_pkey | PRIMARY KEY | id |
| stock_transfers | 2200_41944_1_not_null | CHECK |  |
| stock_transfers | 2200_41944_2_not_null | CHECK |  |
| stock_transfers | 2200_41944_3_not_null | CHECK |  |
| stock_transfers | 2200_41944_4_not_null | CHECK |  |
| stock_transfers | 2200_41944_5_not_null | CHECK |  |
| stock_transfers | 2200_41944_7_not_null | CHECK |  |
| stock_transfers | 2200_41944_8_not_null | CHECK |  |
| stock_transfers | stock_transfers_kind_check | CHECK | CHECK ((kind = ANY (ARRAY['dispatch'::text, 'return'::text, 'adjustment'::text]))) |
| stock_transfers | stock_transfers_status_check | CHECK | CHECK ((status = ANY (ARRAY['pending'::text, 'completed'::text, 'cancelled'::text]))) |
| stock_transfers | stock_transfers_admin_id_fkey | FOREIGN KEY | admin_id -> users(id) |
| stock_transfers | stock_transfers_courier_id_fkey | FOREIGN KEY | courier_id -> users(id) |
| stock_transfers | stock_transfers_created_by_fkey | FOREIGN KEY | created_by -> users(id) |
| stock_transfers | stock_transfers_pkey | PRIMARY KEY | id |
...
| cash_closings_courier_date_unique | cash_closings | `CREATE UNIQUE INDEX cash_closings_courier_date_unique ON public.cash_closings USING btree (courier_id, date)` |
...
| courier_inventory_admin_idx | courier_inventory | `CREATE INDEX courier_inventory_admin_idx ON public.courier_inventory USING btree (admin_id)` |
| courier_inventory_courier_product_idx | courier_inventory | `CREATE INDEX courier_inventory_courier_product_idx ON public.courier_inventory USING btree (courier_id, product_id) WHERE (quantity_remaining > 0)` |
| courier_inventory_lot_idx | courier_inventory | `CREATE INDEX courier_inventory_lot_idx ON public.courier_inventory USING btree (lot_id)` |
| courier_inventory_product_idx | courier_inventory | `CREATE INDEX courier_inventory_product_idx ON public.courier_inventory USING btree (product_id)` |
...
| delivery_attempts_courier_idx | delivery_attempts | `CREATE INDEX delivery_attempts_courier_idx ON public.delivery_attempts USING btree (courier_id)` |
| delivery_attempts_unique | delivery_attempts | `CREATE UNIQUE INDEX delivery_attempts_unique ON public.delivery_attempts USING btree (order_id, attempt_key)` |
...
| inventory_movements_courier_idx | inventory_movements | `CREATE INDEX inventory_movements_courier_idx ON public.inventory_movements USING btree (courier_id) WHERE (courier_id IS NOT NULL)` |
...
| pending_adj_admin_idx | pending_inventory_adjustments | `CREATE INDEX pending_adj_admin_idx ON public.pending_inventory_adjustments USING btree (admin_id, status)` |
| pending_adj_courier_idx | pending_inventory_adjustments | `CREATE INDEX pending_adj_courier_idx ON public.pending_inventory_adjustments USING btree (courier_id)` |
...
| product_lots_lot_number_admin_idx | product_lots | `CREATE UNIQUE INDEX product_lots_lot_number_admin_idx ON public.product_lots USING btree (admin_id, lot_number) WHERE ((active = true) AND (quantity_remaining > 0))` |
...
| stock_transfer_lines_admin_idx | stock_transfer_lines | `CREATE INDEX stock_transfer_lines_admin_idx ON public.stock_transfer_lines USING btree (admin_id)` |
| stock_transfer_lines_lot_idx | stock_transfer_lines | `CREATE INDEX stock_transfer_lines_lot_idx ON public.stock_transfer_lines USING btree (lot_id)` |
| stock_transfer_lines_transfer_idx | stock_transfer_lines | `CREATE INDEX stock_transfer_lines_transfer_idx ON public.stock_transfer_lines USING btree (transfer_id)` |
| stock_transfers_admin_idx | stock_transfers | `CREATE INDEX stock_transfers_admin_idx ON public.stock_transfers USING btree (admin_id)` |
| stock_transfers_courier_idx | stock_transfers | `CREATE INDEX stock_transfers_courier_idx ON public.stock_transfers USING btree (courier_id)` |
| stock_transfers_kind_idx | stock_transfers | `CREATE INDEX stock_transfers_kind_idx ON public.stock_transfers USING btree (admin_id, kind, created_at DESC)` |
| stock_transfers_status_idx | stock_transfers | `CREATE INDEX stock_transfers_status_idx ON public.stock_transfers USING btree (admin_id, status) WHERE (status = 'pending'::text)` |
...
| public.courier_inventory | admin_full_courier_inventory | PERMISSIVE | ALL | {authenticated} | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` |
| public.courier_inventory | block_delete_courier_inventory | RESTRICTIVE | DELETE | {authenticated} | `false` | `—` |
| public.courier_inventory | courier_read_own_inventory | PERMISSIVE | SELECT | {authenticated} | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (courier_id = auth.uid()))` | `—` |
...
| public.delivery_attempts | admin_read_delivery_attempts | PERMISSIVE | SELECT | {authenticated} | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` | `—` |
| public.delivery_attempts | block_delete_delivery_attempts | RESTRICTIVE | DELETE | {authenticated} | `false` | `—` |
| public.delivery_attempts | courier_manage_own_delivery_attempts | PERMISSIVE | ALL | {authenticated} | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (courier_id = auth.uid()))` | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (courier_id = auth.uid()))` |
| public.delivery_attempts | super_admin_full_delivery_attempts | PERMISSIVE | ALL | {authenticated} | `(get_user_role() = 'super_admin'::text)` | `—` |
...
| public.pending_inventory_adjustments | admin_full_pending_adj | PERMISSIVE | ALL | {authenticated} | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` |
| public.pending_inventory_adjustments | courier_read_own_pending_adj | PERMISSIVE | SELECT | {authenticated} | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (courier_id = auth.uid()))` | `—` |
| public.pending_inventory_adjustments | super_admin_full_pending_adj | PERMISSIVE | ALL | {authenticated} | `(get_user_role() = 'super_admin'::text)` | `—` |
...
| public.stock_transfer_lines | admin_full_transfer_lines | PERMISSIVE | ALL | {authenticated} | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` |
| public.stock_transfer_lines | block_delete_transfer_lines | RESTRICTIVE | DELETE | {authenticated} | `false` | `—` |
| public.stock_transfer_lines | courier_read_own_transfer_lines | PERMISSIVE | SELECT | {authenticated} | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (transfer_id IN ( SELECT stock_transfers.id` | `` |
|    FROM stock_transfers |  |  |  |  | `` | `` |
|   WHERE (stock_transfers.courier_id = auth.uid())))) | — |  |  |  | `` | `` |
| public.stock_transfer_lines | super_admin_full_transfer_lines | PERMISSIVE | ALL | {authenticated} | `(get_user_role() = 'super_admin'::text)` | `—` |
| public.stock_transfers | admin_full_stock_transfers | PERMISSIVE | ALL | {authenticated} | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` | `((get_user_role() = 'admin'::text) AND (admin_id = auth.uid()))` |
| public.stock_transfers | block_delete_stock_transfers | RESTRICTIVE | DELETE | {authenticated} | `false` | `—` |
| public.stock_transfers | courier_read_own_transfers | PERMISSIVE | SELECT | {authenticated} | `((get_user_role() = 'courier'::text) AND (admin_id = get_admin_id()) AND (courier_id = auth.uid()))` | `—` |
| public.stock_transfers | super_admin_full_stock_transfers | PERMISSIVE | ALL | {authenticated} | `(get_user_role() = 'super_admin'::text)` | `—` |
...
```sql
CREATE OR REPLACE FUNCTION public.block_courier_deactivation_with_stock()
```
...
### `block_order_items_post_delivery()`
- **Retorna**: trigger
- **Seguridad**: SECURITY INVOKER

```sql
CREATE OR REPLACE FUNCTION public.block_order_items_post_delivery()
```
...
### `deduct_courier_stock(p_courier_id uuid, p_admin_id uuid, p_product_id uuid, p_quantity integer, p_order_item_id uuid, p_order_reference uuid, p_notes text DEFAULT NULL::text)`
- **Retorna**: TABLE(lot_id uuid, allocated_qty integer, unit_cost numeric)
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.deduct_courier_stock(p_courier_id uuid, p_admin_id uuid, p_product_id uuid, p_quantity integer, p_order_item_id uuid, p_order_reference uuid, p_notes text DEFAULT NULL::text)
```
...
### `generate_lot_number(p_product_id uuid, p_admin_id uuid)`
- **Retorna**: text
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.generate_lot_number(p_product_id uuid, p_admin_id uuid)
```
...
### `inbound_stock_with_lot(p_product_id uuid, p_quantity integer, p_unit_cost numeric, p_admin_id uuid, p_lot_number text DEFAULT NULL::text, p_expires_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_no_expiration boolean DEFAULT false, p_supplier text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_suggested_price numeric DEFAULT NULL::numeric)`
- **Retorna**: uuid
- **Seguridad**: SECURITY DEFINER
...
### `refresh_courier_inventory_updated_at()`
- **Retorna**: trigger
- **Seguridad**: SECURITY INVOKER

```sql
CREATE OR REPLACE FUNCTION public.refresh_courier_inventory_updated_at()
```
...
### `register_lot_outbound(p_lot_id uuid, p_quantity integer, p_reason text, p_admin_id uuid, p_customer_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text)`
- **Retorna**: void
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.register_lot_outbound(p_lot_id uuid, p_quantity integer, p_reason text, p_admin_id uuid, p_customer_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text)
```
...
```sql
CREATE OR REPLACE FUNCTION public.return_courier_stock_by_item(p_order_item_id uuid, p_quantity integer, p_admin_id uuid, p_courier_id uuid)
```
...
### `return_from_courier(p_admin_id uuid, p_courier_id uuid, p_lines jsonb, p_kind text DEFAULT 'return'::text, p_notes text DEFAULT NULL::text, p_created_by uuid DEFAULT NULL::uuid)`
- **Retorna**: uuid
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.return_from_courier(p_admin_id uuid, p_courier_id uuid, p_lines jsonb, p_kind text DEFAULT 'return'::text, p_notes text DEFAULT NULL::text, p_created_by uuid DEFAULT NULL::uuid)
```
...
### `transfer_to_courier(p_admin_id uuid, p_courier_id uuid, p_lines jsonb, p_notes text DEFAULT NULL::text, p_created_by uuid DEFAULT NULL::uuid)`
- **Retorna**: uuid
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.transfer_to_courier(p_admin_id uuid, p_courier_id uuid, p_lines jsonb, p_notes text DEFAULT NULL::text, p_created_by uuid DEFAULT NULL::uuid)
```
...
### `update_lot_metadata(p_lot_id uuid, p_admin_id uuid, p_supplier text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_expires_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_lot_number text DEFAULT NULL::text, p_clear_expiration boolean DEFAULT false, p_suggested_price numeric DEFAULT NULL::numeric, p_clear_suggested_price boolean DEFAULT false, p_quantity_received integer DEFAULT NULL::integer, p_correction_reason text DEFAULT NULL::text)`
- **Retorna**: void
- **Seguridad**: SECURITY DEFINER
...
### `validate_courier_has_stock(p_courier_id uuid, p_items jsonb)`
- **Retorna**: jsonb
- **Seguridad**: SECURITY DEFINER

```sql
CREATE OR REPLACE FUNCTION public.validate_courier_has_stock(p_courier_id uuid, p_items jsonb)
```
...
|---------|-------|--------|---------|
| audit_courier_inventory | courier_inventory | DELETE, INSERT, UPDATE | `EXECUTE FUNCTION log_audit()` |
...
| audit_pending_inventory_adjustments | pending_inventory_adjustments | DELETE, INSERT, UPDATE | `EXECUTE FUNCTION log_audit()` |
...
| audit_stock_transfer_lines | stock_transfer_lines | DELETE, INSERT, UPDATE | `EXECUTE FUNCTION log_audit()` |
| audit_stock_transfers | stock_transfers | DELETE, INSERT, UPDATE | `EXECUTE FUNCTION log_audit()` |
| trg_block_courier_deactivation | users | UPDATE | `EXECUTE FUNCTION block_courier_deactivation_with_stock()` |
...
| payments | SI |
| pending_inventory_adjustments | SI |
| product_lots | SI |
| products | SI |
| stock_transfer_lines | SI |
| stock_transfers | SI |
| users | SI |
| zones | SI |
